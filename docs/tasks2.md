...
2. [ ] Rename "Clean" to something clearer, add tooltip explaining what it removes (cookie banners, fundraising popups, chat widgets, newsletter overlays)

## PDF Options — Add to Trial UI
3. [ ] Page size dropdown (A4, Letter, Legal, A3)
4. [ ] Orientation toggle (Portrait / Landscape)
5. [ ] Margins inputs (top, right, bottom, left — with CSS unit examples)
6. [ ] Header template text input (supports pageNumber, totalPages, date, title, url)
7. [ ] Footer template text input (same variables)
8. [ ] Watermark text input + position dropdown (center, top-left, top-right, bottom-left, bottom-right)
9. [ ] Page ranges input (e.g., "1-3", "1,3,5")
10. [ ] Viewport width/height inputs for browser rendering size
