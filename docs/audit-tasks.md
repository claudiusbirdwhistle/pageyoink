...
- [x] A1. Fix cleanup.ts — convert page.evaluate to string-based script
- [x] A2. Fix stealth-adblock.ts — same
- [x] A3. Fix print-fix.ts — same
- [x] A4. Fix readiness.ts — same
- [x] A5. Fix lazy-load.ts — same
- [x] A6. Fix extract.ts — page.evaluate at line 36 and 60
- [x] A7. Add regression test that catches __name issues

---

## 2. BUG FIXES — Things That Are Broken

- [x] B1. Invalid URLs return 500 instead of 400 (e.g., `?url=not-a-url!!!` returns `net::ERR_NAME_NOT_RESOLVED` as 500 — should be 400 "Invalid URL")
- [x] B2. Error message format is inconsistent — some endpoints return `{"error":"..."}`, others return Fastify's `{"statusCode":400,"code":"FST_ERR_VALIDATION","error":"Bad Request","message":"..."}`. Standardize to `{"error":"..."}` everywhere.
- [x] B3. Content tab in landing page shows RAW markdown text (white-space: pre-wrap) instead of rendered/formatted HTML. Headings should look like headings, links should be clickable.
- [x] B4. google.com extraction returns only 11 words of footer links — Readability fails on non-article pages. Should return more useful content or a clear "no article content found" message with full body text fallback.
- [x] B5. 404 pages (e.g., example.com/nonexistent) return 200 with empty/minimal content instead of indicating the page returned an error status code.
...
### Demo UX
- [x] C1. Render markdown as formatted HTML in the Content tab (convert headings, links, code blocks, tables to styled HTML)
- [x] C2. Change default URL from bbc.com (slow, 5+ seconds) to something fast like news.ycombinator.com or stripe.com
- [x] C3. Add loading spinners/skeleton UI for each tab while loading
- [x] C4. Add error states for tabs (if extract or metadata fails, show friendly error, not silent blank)
- [x] C5. Disable "Capture Page" button while capturing (prevent double-clicks)
- [x] C6. Show capture timing ("Captured in 2.3s") to demonstrate speed
- [x] C7. Allow pressing Enter in the URL field to trigger capture

### Content & Credibility
- [x] C8. Add og:image meta tag (screenshot of the landing page or a branded image) for social sharing
- [x] C9. Add twitter:card meta tags
- [x] C10. Add a "How It Works" section — 3 steps: 1) Send a URL, 2) We load it in a real browser, 3) Get any output you need
- [x] C11. Add use case examples: "For AI Agents", "For QA Teams", "For Content Pipelines"
- [x] C12. Add FAQ section answering: What sites work? How fast is it? What about JavaScript? Is it free?
- [x] C13. Add a simple privacy note (we don't store your content, captures are ephemeral)
- [x] C14. Change "Learn more" link in footer to link to /docs

### Visual Polish
- [x] C15. Mobile responsive testing — tabs may not fit on 375px screens
- [x] C16. Test in dark mode (the page IS dark mode, but test in light-mode OS)
- [x] C17. Add subtle animation/transition when tabs switch
- [x] C18. Improve the metadata tab layout — currently a plain grid, could look more like a card

---

## 4. MISSING TESTS — Routes

- [x] D1. Create tests/diff.test.ts
...
- [x] D2. Create tests/trial.test.ts
...
- [x] D3. Create tests/landing.test.ts
...
- [x] E1. Test cleanup.ts (clean mode)
...
- [x] F1. Screenshot quality validation
...
- [x] F3. Markdown extraction quality
...
- [x] F4. Metadata extraction quality
...
- [x] G1. Standardize error responses across all endpoints to `{"error":"message"}` format
...
- [x] G2. Add retry logic to /v1/page for browser crashes (like screenshot/pdf services have)

- [x] G3. Handle non-HTML URLs in /v1/page (PDF files, images, binary)
  - Detect content-type and return error for extract/markdown but allow screenshot/pdf

- [x] G4. Handle empty pages gracefully
  - Extract should return `{"content":"","wordCount":0}` not an error
  - Metadata should work even on blank pages

- [x] G5. Report upstream HTTP status in extract response
...
- [x] H1. Trial PDF SSRF: uses dynamic import (`await import("../utils/ssrf.js")`) — convert to static import at top of file
- [x] H2. Custom error handler to prevent Fastify from leaking stack traces in production
- [x] H3. Add Helmet-style security headers (X-Content-Type-Options, X-Frame-Options, etc.)
- [x] H4. Validate proxy URL format more strictly (must be host:port or http://host:port)
- [x] H5. Add rate limit response headers that tell users when the limit resets
...
|----------|-------|----------|--------|
| __name bugs | 7 | CRITICAL | **DONE** |
| Bug fixes | 5 | CRITICAL | **DONE** |
| Landing page | 18 | HIGH | **DONE** |
| Missing route tests | 3 | HIGH | **DONE** |
| Missing service tests | 3 | HIGH | **DONE** |
| Integration tests | 5 | HIGH | **DONE** |
| Error handling | 5 | MEDIUM | **DONE** |
| Security | 5 | MEDIUM | **DONE** |
| Build/ops | 5 | MEDIUM | Not started |
| SDK/docs | 6 | MEDIUM | Not started |
| Content quality | 3 | MEDIUM | Not started |
| MCP server | 5 | LOW | Not started |
| **TOTAL** | **70** | | **51 done** |
